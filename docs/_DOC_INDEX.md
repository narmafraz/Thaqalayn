...
| Document | Status | Summary |
|----------|--------|---------|
| [SPARK_AI_CONTENT_ROADMAP.md](SPARK_AI_CONTENT_ROADMAP.md) | **ACTIVE** (ideas 1-8 unscheduled; item 9 ✅ COMPLETE core 2026-09-06 — 99.8% of scraped English translation-ids chunk-aligned + merged; item 10 in progress) | Candidate AI-content workloads for the DGX Spark at $0 cost: 10-lang translation gap fill, word-by-word gap/accuracy review, robust narrator canonicalisation (stable IDs, resolve "his father" refs), book+narrator research enrichment in 10 langs, chapter-name translation completeness (subsumes CHAPTER_TRANSLATION_GAP cost model), chapter summaries with point-level dedup, content-aware narrator-insight corroboration (same-topic clusters × disjoint chains), cross-corpus similar-narration detection, chunk-aligning scraped translations to AI chunks (built, pilot-verified; strict validation + full-corpus run remain), and word-regen determinism + wrong-lemma-link gloss quality (fixes in working tree; principled tiebreak + full-rebuild validation remain). Includes sequencing sketch. |
