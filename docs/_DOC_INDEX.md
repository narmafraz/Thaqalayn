...
| Document | Status | Summary |
|----------|--------|---------|
| [SPARK_AI_CONTENT_ROADMAP.md](SPARK_AI_CONTENT_ROADMAP.md) | **ACTIVE** (idea collection; nothing scheduled) | Candidate AI-content workloads for the DGX Spark at $0 cost: 10-lang translation gap fill, word-by-word gap/accuracy review, robust narrator canonicalisation (stable IDs, resolve "his father" refs), book+narrator research enrichment in 10 langs, chapter-name translation completeness (subsumes CHAPTER_TRANSLATION_GAP cost model), chapter summaries with point-level dedup, content-aware narrator-insight corroboration (same-topic clusters × disjoint chains), and cross-corpus similar-narration detection. Includes sequencing sketch. |
