...
10. [Topic Browsing & Cross-Referencing](#10-topic-browsing--cross-referencing)
11. [Appendix: Data Sources Reference](#appendix-data-sources-reference)
...
## 10. Topic Browsing & Cross-Referencing

### 10.1 Overview

Three new fields added to the AI pipeline schema (v2.0.0) enable a suite of cross-referencing features inspired by Bihar al-Anwar's chapter organization and traditional Islamic topic indexing.

### 10.2 Data Sources (Generated by AI Pipeline)
...
**Data requirement:** `index/topics.json` built by inverting the `topics` field across all hadiths.

### 10.4 Phrase Cross-Reference

When a user reads a hadith containing a phrase like "يوم القيامة" (Day of Resurrection), they can click it to see all other hadiths using that phrase. Phrases are highlighted in the reading view.

**Data requirement:** `index/phrases.json` built by inverting the `key_phrases` field across all hadiths.

### 10.5 Similar Hadiths

"Related Narrations" section on each hadith page, showing hadiths that are thematically similar or represent variant transmissions of the same tradition (takhrij).

**Data requirement:** `index/similar_hadiths.json` built by combining:
1. `similar_content_hints` themes (LLM-seeded)
2. `topics` overlap (hadiths sharing 2+ topics)
3. `key_phrases` overlap
4. Arabic-Triplet-Matryoshka-V2 embedding similarity
5. Narrator chain overlap (3+ shared narrators)

### 10.6 Implementation Notes

- All post-processing indexes are built at **generator time** (build-time computation), served as static JSON
- No runtime cost — indexes are static files on Netlify CDN
- Topic taxonomy defined in `ThaqalaynDataSources/ai-pipeline-data/topic_taxonomy.json`
- Phrase dictionary seeded from `ThaqalaynDataSources/ai-pipeline-data/key_phrases_dictionary.json`
- Angular components needed: `TopicListComponent`, `TopicDetailComponent`, `PhraseDetailComponent`, `SimilarHadithsComponent`
