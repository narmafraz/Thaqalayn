...
| **N7** | **Finish the alignment tail (thawab-al-amal + uyun)** | The full run was stopped at ~98.5% on 2026-09-06 (user needed the Spark for a vLLM restart). ~1,000 eligible verses remain (thawab-al-amal tail + uyun-akhbar-al-rida). Resume: `ssh` to the Spark, `tmux new-session -d -s align "bash ~/align_full.sh 2>&1 | tee -a ~/align_full.log"` — fully resumable, only missing ids run. Then pull artifacts (tar command in the handoff doc), re-run reports, commit the DataSources delta, re-run `merge_chunk_alignment()` + `scripts/verify_alignment_merge.py`, commit the Data delta. |
