...
| **Chunk-align scraped translations — CORE COMPLETE (2026-09-06/07)**: full-corpus Spark run aligned **41,057 of 41,155 English translation-ids (99.8%)** across 21 books after two validation rounds over all 19 translator styles; merged corpus-wide into ThaqalaynData (54,668 files; integrity sweep 41,155/41,155 OK); verified with full suites (Karma 876/876, Playwright 303/304 — 1 known pre-existing PAR-04) + per-style UI spot checks. 98 quarantined ids keep the flat view by design. See N1 (record) and N7-N10 (follow-ups). | infrastructure: generator `e938070`→`dd486b7`, frontend `663ca71`→`1fcdaef` (07-05); validation+salvage: generator `0485f1b`→`ff54c8a`; search-builder fix `e907a97`; artifacts DataSources `1e6bfe2f6`; merge ThaqalaynData `bcad2e28a86` (all local, unpushed) |

### Newly captured outstanding items (were in session transcripts/memory only, previously in no roadmap)

| # | Item | Detail |
|---|------|--------|
| **N1** | **✅ COMPLETE (core) 2026-09-06 — Spark item 9, chunk-align scraped translations** | Finished over 2026-08-16 → 09-06: strict verbatim validation (NFC, whitespace-collapsed), deterministic number-prefix handling, cut-point reslice salvage, placement accuracy gate (off-by-one / wrong-verse / stub detection), best-effort sentence-DP for abridged translations, two validation rounds over **all 19 translator styles** (99.5-99.6%), then the **full-corpus Spark run: 41,057 of 41,155 translation-ids aligned (99.8%)** across 21 books, merged into ThaqalaynData (54,668 files; integrity sweep 41,155/41,155 OK). Quarantined 98 = by-design (abridged Sarwar summaries, wrong-verse attachments, stubs) — they keep the flat block view. ThaqalaynSearch builder patched to read sister `chunk_translations` (`e907a97`) so a search regen doesn't lose human-translation text. Residuals: **N7** (thawab/uyun tail), future language-file migration (Spark roadmap item 9 §3). Operational handoff: `scripture/handoff/ALIGN_SCRAPED_HANDOFF.md`. |
...
| **N7** | **✅ COMPLETE 2026-09-07 — alignment tail (thawab-al-amal + uyun)** | Tail run finished 01:15 UTC (781 ids; 5 old quarantines also recovered). **Final corpus: 41,838/41,931 translation-ids aligned (99.8%), 93 quarantined by design.** Artifacts DataSources `dc61a487e`, merge ThaqalaynData `9b3875fc805`, integrity sweep 41,931/41,931 OK — both pushed; deploy verified live. The align-scraped project is fully DONE except the future language-file migration (Spark item 9 §2) and the N8-N10 follow-ups. |
| **N8** | **✅ COMPLETE 2026-09-07 — Sarwar misattachment root cause fixed in the generator** | Root cause was NOT the scraper: `kafi_sarwar.add_chapter_content` attached Sarwar sections **positionally**, so any chapter where thaqalayn.net's section order/numbering diverged from hubeali's verse order misattached translations (245 verses corpus-wide). Fix (generator commits `72de207`→`3aa20c9`): prefix-guided attachment (parse leading hadith numbers, `use_prefix` when numbering starts ≤2), Arabic-similarity verification (diacritics-stripped token Jaccard, refuse attach < 0.25), inferred slots for unnumbered sections, and `_clear_sarwar()` before re-attach (stale carryover from the previous build's complete/al-kafi.json). Also purged ~15 phantom appended hadith from an old append bug (al-kafi total 15385→15369, test updated `0f5fc3f`). Full regen → 245 mismatches → 0; realigned affected ids on Spark (15 aligned). **34 remaining quarantines are by design**: al-kafi 1:4:104/108 numbering is wrong in thaqalayn.net's own source (their internal numbering fault) — translations correctly kept unattached rather than guessed. 26 new parser tests. |
| **N9** | **Scrape an English tahdhib al-ahkam source** | Tahdhib has NO scraped English at all (`translations: []` — discovered by alignment eligibility scans), so it's invisible to alignment AND to human-translation search/display. Bab ul Qaim PDFs cover vols 1-3 (babulqaim.com / almuntazar.ca, see generator CLAUDE.md source research). New scraper + parser work; after ingestion, alignment picks it up automatically. |
| **N10** | **Persian (fa.*) quran alignment — pilot needed (BACKLOG, user decision 2026-09-06)** | The quran carries 11 scraped Persian translations (fa.ansarian … fa.sadeqi). Aligning them cross-lingually (Persian text vs English chunk references) needs its own pilot + validation metrics (the current placement gate is English-content-word based). Scoped as a future Spark batch; do not fold into the English pipeline. |
...
| 9 | Chunk-align scraped translations to AI chunks | **✅ COMPLETE (core), 2026-09-06** — see N1; residual: thawab/uyun tail (N7) + language-file migration (future) |
| 10 | Word regen determinism + surface gloss quality | **IN PROGRESS** — see N2 |
| — | *New follow-ups from item 9's full run:* | see N7–N10 below |
