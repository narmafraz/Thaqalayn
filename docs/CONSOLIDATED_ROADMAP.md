...
| **N7** | **Finish the alignment tail (thawab-al-amal + uyun)** | **IN FLIGHT** — the tail run was resumed on the Spark 2026-09-07 after the vLLM restart (tmux `align`, `~/align_full.log`; 475+ new ids aligned within the first hour). When it prints `FULL RUN COMPLETE`: pull artifacts (tar command in the handoff doc), re-run the two reports, commit the DataSources delta, re-run `merge_chunk_alignment()` + `scripts/verify_alignment_merge.py` (must exit 0), commit the Data delta. If the run ever dies, re-launching `bash ~/align_full.sh` resumes losslessly. |
| **N8** | **Sarwar hadith-numbering offset in source data (upstream audit)** | The alignment gate exposed scraped Sarwar translations attached to the WRONG verse — off-by-one hadith numbering clustered in al-kafi 1:4:104 and 1:4:108 (~33 ids caught + quarantined). This is a scraper/source-data bug independent of alignment: audit the ThaqalaynAPI Sarwar data for those chapters (and scan corpus-wide for number-prefix vs verse-index mismatches), fix attachments, then re-run alignment for the corrected verses with `--attempt-quarantined`. |
