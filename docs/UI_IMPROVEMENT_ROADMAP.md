...
> **Status:** FB-01 (icon revert), FB-01b (combined view), FB-02 (inline AI badges), FB-03 (word popup), FB-05 (book authors), FB-06 (mobile search) all implemented (2026-03-10). FB-04 (narrator extraction) research complete — requires generator-side changes, see detailed plan below. FB-07 (UX review) pending — requires review resources.
>
...
**Status:** RESEARCH COMPLETE — Generator changes required. No frontend work needed.
...
**Current state:**
- Only Al-Kafi has `narrator_chain` data with clickable narrator links (generated by `kafi_narrators.py` using Arabic regex patterns specific to Al-Kafi's hadith text format)
- The Angular frontend already renders `narrator_chain.parts` with clickable links when present — no frontend changes needed
- The AI pipeline already generates `isnad_matn` data (isnad/matn separation, narrator identification) for books with AI content (e.g., al-istibsar), and the frontend already renders this via the isnad separation toggle

**Available data sources for non-Kafi books:**
- **ThaqalaynAPI source data** (`ThaqalaynDataSources/scraped/thaqalayn_api/`): 25 books have `thaqalaynSanad` and `thaqalaynMatn` fields providing English-language isnad/matn separation. The `thaqalayn_api.py` parser currently **ignores** these fields entirely — it only extracts `arabicText`, `englishText`, `frenchText`, gradings, and `URL`.
- **AI pipeline `isnad_matn`**: For books that have AI content, the pipeline generates structured narrator identification with Arabic names, English names, roles, confidence levels. This data lives in the AI response JSON files but is only embedded in verse-detail JSON, not in the chapter-level `narrator_chain` structure.

**Why this is not a frontend task:**
- The `verse.narrator_chain` field (used by the clickable-link rendering) is only populated by `kafi_narrators.py` during data generation
- Non-Kafi books have no `narrator_chain` in their generated JSON at all
- The frontend code path that renders narrator chains is already fully functional

#### Implementation Plan (Generator-Side)

**Phase A: English isnad/matn text separation (Effort: S, < 1 day)**
Modify `ThaqalaynDataGenerator/app/thaqalayn_api.py` `build_verse()` to extract `thaqalaynSanad`/`thaqalaynMatn` from the source data and store them. Options:
1. Store as `narrator_chain.text` (the raw English sanad text) — simplest, gives the frontend something to display
2. Store English sanad separately and use it to split the Arabic text at the corresponding boundary

**Phase B: Arabic narrator name extraction (Effort: M, 2-3 days)**
Create a generic Arabic narrator extraction module (similar to `kafi_narrators.py` but book-agnostic):
- Use common Arabic narrator transmission patterns (`حدثنا`, `عن`, `أخبرنا`, etc.) that work across all hadith books
- Extract individual narrator names from the Arabic isnad text
- Build `narrator_chain.parts` with `kind: "plain"` for all entries initially

**Phase C: Narrator ID matching (Effort: L, 3-5 days)**
Match extracted narrator names against the existing narrator database (`people/narrators/`):
- Fuzzy matching on Arabic names (handling diacritics variation)
- Assign `kind: "narrator"` and `path` for matched names
- Add new narrators to the index for unmatched names
- Leverage AI pipeline `narrator_templates.json` (1,074 pre-validated entries) for initial matching

**Phase D: AI-pipeline narrator backfill (Effort: M, ongoing)**
For books processed by the AI pipeline, use the structured `isnad_matn.narrators` data to create precise `narrator_chain.parts` with confidence levels.
