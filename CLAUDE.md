...
- **`e2e/tests/words.spec.ts` needs a second data server.** Word pages read `wordsApiBaseUrl` (`http://localhost:8889/` in dev), which is the `ThaqalaynWords` repo — `cd ../ThaqalaynWords && py serve.py`. Without it all 8 words specs fail on missing data, which looks like a UI regression and is not one.
- **Known pre-existing failure:** `phase6-content-parity.spec.ts` "PAR-04: action buttons are visible inline, no overflow menu" fails against localhost (verified at `144e98b`). Not a regression — check it against the base commit before chasing it.
- **The search specs are load-sensitive.** `search-fulltext`, `search-load-more` and `search-regression` hit the remote Pagefind index (`ThaqalaynSearch`), so under a full parallel run they time out and report as failed or flaky; they pass when run on their own. Re-run a search failure in isolation before treating it as real.
